...
class Pos {
  constructor(filename, lineno, column, v8Condition) {
    this.filename = filename;
    this.lineno = lineno;
    this.column = column;
    this.v8Condition = v8Condition;
...
class ScriptNotifier {
  ...
  registerNotification(aug, filename, fileHash, includeExternals, lineno, lineCrc32, lineUnique, callback, removed) {
    ...
    if (matchInfos.length > 0) {
      ...
      for (let matchInfo of matchInfos) {
        if (!warningSent && null !== matchInfo.script && undefined !== matchInfo.script) {
          const script = matchInfo.script;

          if (script.localScriptTranspiled && !script.hasSourceMaps) {
            aug.sendWarning(new _RookError.default(new _exceptions.RookSourceMapMissing(matchInfo.script.filename)));
            warningSent = true;
          } else if (script.typescriptMismatch) {
            aug.sendWarning(new _RookError.default(new _exceptions.RookTypescriptSourceMapMismatch()));
            warningSent = true;
          }
        }
      ...
      for (let scriptId of Object.keys(this.scripts)) {
        ...
        if (!warningSent) {
          if (script.localScriptTranspiled && !script.hasSourceMaps) {
            aug.sendWarning(new _RookError.default(new _exceptions.RookSourceMapMissing(filename)));
            warningSent = true;
          } else if (script.typescriptMismatch) {
            aug.sendWarning(new _RookError.default(new _exceptions.RookTypescriptSourceMapMismatch()));
            warningSent = true;
  ...
  addScript(scriptId, filename, source, sourceMapURL) {
    if (null == filename || "" === filename || this.scripts.hasOwnProperty(scriptId)) {
      return;
    }

    const scriptWrapper = new _ScriptWrapper.default(scriptId, filename, source, sourceMapURL);
    ...
    for (let augId of Object.keys(this.pendingNotifications)) {
      ...
      if (null !== matchInfo) {
        ...
        if (scriptWrapper.localScriptTranspiled && !scriptWrapper.hasSourceMaps) {
          notification.userWarnings.sendWarning(new _RookError.default(new _exceptions.RookSourceMapMissing(scriptWrapper.filename)));
        } else if (scriptWrapper.typescriptMismatch) {
          notification.userWarnings.sendWarning(new _RookError.default(new _exceptions.RookTypescriptSourceMapMismatch()));
        }
      }
    }
...
class AugsHolder {
  ...
  addAug(filename, lineno, column, aug, v8Condition) {
    _logger.logger.info("Setting breakpoint at %s:%s:%s", filename, lineno, column);

    let pos = new Pos((0, _utils.canonizeFileName)(filename), lineno, column, v8Condition);
  ...
  removeAugTemporarily(augId, duration) {
    ...
    if (this.timers[pos.key] != null) {
      clearTimeout(this.timers[pos.key]);
      delete this.timers[pos.key];
    }

    this.timers[pos.key] = setTimeout(() => {
      try {
    ...
    this.timers[pos.key].unref();
  }

  removeAug(augId, removeFromPositions = true) {
    if (!this.positions.hasOwnProperty(augId)) {
      return;
...
class DebuggerService extends _IService.IService {
  ...
  addAug(matchInfo, aug, v8Condition = undefined) {
    ...
    this.augsHolder.addAug(matchInfo.script.filename, matchInfo.lineno, matchInfo.column, aug, v8Condition);
  }
  ...
  removeAugfromInspector(augId) {
    if (null !== this.augsHolder) {
      this.augsHolder.removeAugfromInspector(augId);
    }
  }
  ...
  getDebugger() {
    if (config.DebuggerService.IS_DEPRECATED) {
      throw new _exceptions.RookUnsupportedOldNodeVersion(process.version);
    }

    if (config.DebuggerService.IS_NON_LTS) {
      throw new _exceptions.RookUnsupportedNonLTSNodeVersion(process.version);
    }

    if (!config.DebuggerService.IS_SUPPORTED) {
