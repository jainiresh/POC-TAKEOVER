...
class InspectorDebug {
  constructor(userLine, scriptLoaded, getScript, dbgService) {
    ...
    this.paused = false;
    this.is_12_5_or_above = InspectorDebug.validate_version(12, 5);
    this.dumpedAugNotFoundJSON = false;
    (0, _IgnoredNodeModules.RemoveWhitelistedFromBlacklist)();
    this.startSession();
  }
  ...
  async startSession() {
    ...
    this.session.on('Debugger.scriptParsed', ({
      ...
      try {
        ...
        if (scriptURL === "" || (0, _IgnoredNodeModules.isBlackListedModule)(scriptURL) && !(0, _IgnoredNodeModules.isDummyModule)(scriptURL)) {
          if (this.is_12_5_or_above) {
        ...
        if (config.InstrumentationConfig.QUICK_START && params.sourceMapURL) {
          if (config.LoggingConfiguration.ROOKOUT_SOURCEMAP_DEBUG) {
            _logger.logger.debug("Using `params.sourceMapURL` for script id:", params.scriptId, ". Quick start is enabled");
          }

          this.scriptLoaded(params.scriptId, scriptURL, null, params.sourceMapURL);
          return;
        }

        if (config.LoggingConfiguration.ROOKOUT_SOURCEMAP_DEBUG) {
          _logger.logger.debug("Asking source for script id:", params.scriptId);
        }
        ...
        this.post('Debugger.getScriptSource', {
          ...
          try {
            if (config.LoggingConfiguration.ROOKOUT_SOURCEMAP_DEBUG) {
              _logger.logger.debug("Adding script:", scriptURL, "id:", params.scriptId);
            }

            this.scriptLoaded(params.scriptId, scriptURL, scriptSource.scriptSource, null);
          } catch (error) {
    ...
    this.session.on('Debugger.paused', ({
      params
    }) => {
      ...
      try {
        if (params.reason === 'other' && params.hitBreakpoints.length === 0) {
          _logger.logger.error(`Got called with no breakpoints`);

          if (!this.dumpedAugNotFoundJSON) {
            _logger.logger.error(`No breakpoints message: ${JSON.stringify(params)}`);

            this.dumpedAugNotFoundJSON = true;
          }

          if (config.InstrumentationConfig.EMPTY_HITBREAKPOINTS_FAILSAFE) {
            this.dbgService.close(true);
          }
        ...
        this.paused = true;
        const pos = this.breakpointsToPositions[params.hitBreakpoints[0]];

        if (!pos) {
          _logger.logger.error(`Aug not found! ${JSON.stringify(params)}`);
        } else {
          this.userLine(pos, new _InspectorStackNamespace.default(this, params.callFrames));
        }
      } catch (error) {
    ...
    this.sync_post('Debugger.setAsyncCallStackDepth', {
      maxDepth: 0
    });

    if (InspectorDebug.validate_version(12, 11) && !InspectorDebug.validate_version(18)) {
      this.sync_post('Debugger.setAsyncCallStackDepth', {
        maxDepth: 1
      });
    }

    this.sync_post('Debugger.setBreakpointsActive', {
      active: true
    });

    if (InspectorDebug.validate_version(14) && !InspectorDebug.validate_version(18)) {
  ...
  cleanPausesMemory() {
    ...
    if (this.pausesToClean > CLEAN_EVERY_X_CALLS) {
      setImmediate(() => {
        try {
  ...
  cleanDebuggerMemory() {
    ...
    if (this.callsToClean > CLEAN_EVERY_X_CALLS && !this.paused) {
  ...
  setBreak(pos) {
    ...
    _logger.logger.debug(`setBreakpointByUrl ${scriptURL}:${lineNumber}:${columnNumber}`);

    const setBreakpointParams = {
      url: scriptURL,
      lineNumber,
      columnNumber
    };
    const condition = pos.v8Condition;

    if (condition !== undefined) {
      setBreakpointParams.condition = condition;
    } // While this function call looks async, in reality the callback will be called before it returns,
    // so we simplify the code


    this.post('Debugger.setBreakpointByUrl', setBreakpointParams, (error, response) => {
      result.error = error;
    ...
    if (undefined === result.response.breakpointId || null === result.response.breakpointId || "" === result.response.breakpointId) {
      _logger.logger.error(`Invalid breakpointId`);
  ...
  sync_post(message_id, params) {
    let err = new Error("Post behaved in an async manner");
    this.post(message_id, params, cb_err => err = cb_err);
  ...
  async close() {
    if (this.session) {
      ...
      this.sync_post('Debugger.disable');

      try {
        this.sync_post('Debugger.resume'); // just in case
      } catch (e) {}
  ...
  static validate_version(major, minor = 0, patch = 0) {
    /**
