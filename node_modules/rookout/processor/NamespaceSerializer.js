...
class NamespaceSerializer extends _NamespaceSerializerBase.default {
  constructor(useStringCache = false) {
    super(useStringCache);
  ...
  dump(namespace, variant, logErrors = true) {
    try {
      this.dumpUnsafe(namespace, variant, logErrors);
    } catch (e) {
  ...
  unsafeDumpJSObject(obj, variant, currentDepth, dumpConfig, logErrors) {
    ...
    if (this.isFloat(obj) || Number.isInteger(obj)) {
  ...
  dumpLogRecordNamespace(namespace, variant, logErrors) {
    variant.writeVariantType(variant_pb.Variant.Type.VARIANT_LIVETAIL);
    let liveTailMessage = new variant_pb.Variant.LiveTailMessage();
    namespace.dumpToVariant(liveTailMessage);
    variant.writeLiveTailValue(liveTailMessage, (livetail, writer) => {
  ...
  dumpInspectorTracebackNamespace(namespace, variant) {
    if (!this.useStringCache) {
      ...
      let self = this;
      namespace.dump(variant, str => self.getStringIndexInCache(str));
    }
  }
