...
class Backoff {
  ...
  async afterDisconnect() {
    if (this._connectTime !== null && Date.now() > this._connectTime + config.AgentComConfiguration.RESET_BACKOFF_TIMEOUT * 1000) {
      this._resetBackoff();
    }

    this._connectTime = null;

    _logger.logger.debug(`Backing off ${this._nextBackoff} seconds`);
  ...
  _timeout(ms) {
    return new Promise(resolve => {
      let timeout = setTimeout(resolve, ms * 1000);
