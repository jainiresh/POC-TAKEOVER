...
var _StreamingEnvelope = _interopRequireDefault(require("./StreamingEnvelope"));
...
class AgentComWs {
  ...
  waitForReady(keepRef) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once("InitialAugsCommand", async () => {
        clearTimeout(timer);
        this._rejectWaitForReady = null;
        resolve();
      });
  ...
  sendUserMessage(augId, messageId, args) {
    let streamingEnvelope;

    try {
      streamingEnvelope = new _StreamingEnvelope.default(this.id, augId, messageId, args);
    } catch (e) {
      let errorMessage = "Failed to serialize user message";
      throw new _RookError.default(e, errorMessage);
    }

    this.addEnvelope(streamingEnvelope);
  } ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  addEnvelope(envelope) {
    if (this.isQueueFull(envelope.length)) {
      throw new _exceptions.RookOutputQueueFull(envelope.length, this._pendingMessagesSize, this._pendingMessages.length);
    }
  ...
  isQueueFull(nextMessageSize = 0) {
    let extraMessagesCount = 0;

    if (nextMessageSize > 0) {
      extraMessagesCount = 1;
    }

    if (this._pendingMessages.length + extraMessagesCount > config.AgentComConfiguration.MAX_QUEUED_MESSAGES) {
      return true;
    } // TODO: Change to nextMessageSize


    return this._pendingMessagesSize + extraMessagesCount >= config.AgentComConfiguration.MAX_QUEUED_MESSAGES_SIZE;
  } ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    while (this._running) {
      try {
        ...
        if (this._print_on_initial_connection) {
          ...
          utils.quietPrint("[Rookout] Successfully connected to controller.");

          _logger.logger.debug("[Rookout] Agent ID is " + this.id);
        }
        ...
        if (e.statusCode === 400 && this._rejectWaitForReady) {
          this._rejectWaitForReady(new _exceptions.RookWebSocketError(e.statusCode));
        }

        _logger.logger.warn(`Connection failed; reason = ${JSON.stringify(e)}`);
      }
  ...
  async sendPendingMessagesRaw() {
    ...
    try {
      ...
      while (this._running && this._pendingMessages.length > 0) {
        ...
        try {
          const buffer = pendingMessage.getBuffer !== undefined ? pendingMessage.getBuffer() : pendingMessage;
          await this._client.send(buffer);
        } catch (e) {
