...
exports.RookAnchorLinesServiceNotFound = RookAnchorLinesServiceNotFound;
exports.RookAssertTrackingServiceNotFound = RookAssertTrackingServiceNotFound;
...
exports.RookInvalidOperation = RookInvalidOperation;
exports.RookInvalidOptions = RookInvalidOptions;
exports.RookInvalidRateLimitConfiguration = RookInvalidRateLimitConfiguration;
...
exports.RookOutputQueueFull = RookOutputQueueFull;
...
exports.RookSourceMapMissing = RookSourceMapMissing;
exports.RookSourceMissing = RookSourceMissing;
exports.RookTypescriptSourceMapMismatch = RookTypescriptSourceMapMismatch;
exports.RookUnsupportedLocation = RookUnsupportedLocation;
exports.RookUnsupportedNodeVersion = RookUnsupportedNodeVersion;
exports.RookUnsupportedNonLTSNodeVersion = RookUnsupportedNonLTSNodeVersion;
exports.RookUnsupportedNotLTS = RookUnsupportedNotLTS;
exports.RookUnsupportedOldNodeVersion = RookUnsupportedOldNodeVersion;
exports.RookUnsupportedRuntime = RookUnsupportedRuntime;
exports.RookWebSocketError = RookWebSocketError;
...
function RookDebuggerPermanentlyClosedException() {
  ToolException.call(this, 'Disabled for safety reasons (too many scripts or an unidentified breakpoint)');
}
...
function RookUnsupportedOldNodeVersion(version) {
  ToolException.call(this, util.format("Old Node version %s, not supported anymore", version), {
    version: version
  });
}

setExceptionPrototype(RookUnsupportedOldNodeVersion);

function RookUnsupportedNonLTSNodeVersion(version) {
  ToolException.call(this, util.format("Non-LTS node version %s, not supported", version), {
    version: version
  });
}

setExceptionPrototype(RookUnsupportedNonLTSNodeVersion);
...
function RookInvalidRateLimitConfiguration(config) {
  ToolException.call(this, `Got an invalid value for the rate limit. (got " ${config} ") expected XX/YY or XX\\YY, where XX < YY`, {
    config: config
  });
}

setExceptionPrototype(RookInvalidRateLimitConfiguration);

function RookRuleAugRateLimited() {
...
setExceptionPrototype(RookRuleAugRateLimited);

function RookRuleGlobalRateLimited() {
  ToolException.call(this, 'Breakpoint was global rate limited. For more information: https://docs.rookout.com/docs/breakpoints-tasks.html#rate-limiting');
}

setExceptionPrototype(RookRuleGlobalRateLimited);

function RookRuleRateLimited() {
  if (config.RateLimitConfiguration.USING_GLOBAL_RATE_LIMITER) {
    return new RookRuleGlobalRateLimited();
  }

  return new RookRuleAugRateLimited();
}
...
function RookSourceMapMissing(filename) {
  ToolException.call(this, util.format('Source maps not found for (possibly) transpiled file: %s. Did you embed them?', filename), {
    filename: filename
  });
}

setExceptionPrototype(RookSourceMapMissing);

function RookTypescriptSourceMapMismatch() {
  ToolException.call(this, 'Detected incorrect source maps configuration for typescript files. Please refer to the docs');
}

setExceptionPrototype(RookTypescriptSourceMapMismatch);
...
function RookWebSocketError(httpStatus) {
  ToolException.call(this, util.format("Received HTTP status %d from the controller, please make sure WebSocket is enabled on the load balancer.", httpStatus));
}

setExceptionPrototype(RookWebSocketError);
...
function RookOutputQueueFull(messageSize = 0, queueCurrSize = 0, queueCurrMsgs = 0) {
  let msg = util.format('Breakpoint trigger but output queue is full. Message size = %d, ' + 'queueCurrSize = %d, queueCurrMsgs = %d. Data collection will be disabled until the queue has emptied.', messageSize, queueCurrSize, queueCurrMsgs);
  ToolException.call(this, msg);
}

setExceptionPrototype(RookOutputQueueFull);
...
function RookAssertTrackingServiceNotFound() {
  ToolException.call(this, 'Assert tracking breakpoint was ignored because the assert tracking service was not found.');
}

setExceptionPrototype(RookAssertTrackingServiceNotFound);

function RookAnchorLinesServiceNotFound() {
  ToolException.call(this, 'Anchor lines breakpoint was ignored because the anchor lines service was not found.');
}

setExceptionPrototype(RookAnchorLinesServiceNotFound);

function RookInvalidOperation(operationName) {
  ToolException.call(this, util.format("Invalid operation in aug configuration: %s", operationName));
}

setExceptionPrototype(RookInvalidOperation);